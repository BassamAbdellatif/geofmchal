...
def main():
    ...
    print(f"Loading embedding files from: {args.test_embeddings_dir}")
    # Recursively find all .tif files
    emb_files = glob.glob(os.path.join(args.test_embeddings_dir, "**", "*.tif"), recursive=True)
    if not emb_files:
        raise RuntimeError("No .tif files found in --test-embeddings-dir.")
