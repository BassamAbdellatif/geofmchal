...
def parse_args():
    ...
    parser.add_argument("--experiment-name", type=str, required=True,
                        help="Name of the experiment (must match the run folder and zip created by package.py).")
...
def upload_submission():
    ...
    if not os.path.exists(FILE_PATH):
        ...
        print(f"   → Run package.py --experiment-name {args.experiment_name} first.")
        return
    ...
    with sync_playwright() as p:
        ...
        # Derive name/description from the experiment name (not the zip filename)
        exp_name = args.experiment_name
        suffix_label = "_tta" if args.tta else ""
        form_name = f"submission_{exp_name}{suffix_label}"
        form_desc = exp_name
        print(f"📝 Filling Name: '{form_name}' | Description: '{form_desc}'")
